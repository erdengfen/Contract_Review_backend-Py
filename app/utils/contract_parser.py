...
class ContractParser:
    ...
    @staticmethod
    async def read_pdf_first_page(file_path: str) -> str:
    ...
    async def extract_parties_with_llm(file_path: str, llm) -> Dict[str, str]:
        ...
        try:
            ...
            # 1.根据文件类型读取
            if ext == ".pdf":
                text = await ContractParser.read_pdf_first_page(file_path)
            elif ext == ".docx":
                text = await ContractParser.read_docx_text(file_path)
            else:
