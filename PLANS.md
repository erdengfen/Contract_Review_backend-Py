...
- 文件接收、原始文件存储、文件记录入库和权限控制归属 backend；agent 只接收 `file_id`、`storage_uri`、`save_path` 等稳定文件引用，并基于这些引用读取文件后执行感知、解析、分块和审阅。
...
- 文件上传接收、原始文件存储、文件元数据入库、文件访问权限校验和稳定文件引用生成。
...
- 基于 backend 传入的 `file_id`、`storage_uri`、`save_path` 等稳定文件引用读取待处理文件，不负责接收上传文件流，不负责原始文件入库。
...
- `backend` 调用 `agent` 时只传当前 FastAPI 入参、登录用户、会话、文件记录、稳定文件引用和已存在处理流程能提供的业务参数，不传 API key、OpenAI client、LLM 实例或模型 SDK 对象。
...
- √ 在 `src/agent/contracts/` 实现 Step 3 Pydantic 契约类，不依赖旧 `app/` 模块。
- √ 补充契约层结构测试，不接真实模型和外部服务。

### Step 4：文档感知与存储安全

- √ 实现存储阶段提示词注入防护规则，避免合同正文影响系统 prompt 和工具调用。
- √ 实现 DOC/DOCX 归一化策略与分流标记，不在感知层执行实际转换。
- √ 实现 PDF 物理类型判断逻辑，将扫描件 PDF 和普通 PDF 分流。
- √ 实现文件感知层输出结构，保留文件类型、页码、版面序号和解析置信度。
- √ 使用样例文件做结构验证，确认不改变上传接口返回字段。
- 【】待完成 FastAPI 上传接口到新 `src/agent/document_intake/` 感知入口的接线；当前旧接口仍在 `app/router/contract.py` 接收文件。
- 【】待完成新 backend 侧真实文件存储逻辑；当前 agent 感知层只接收已保存文件的 `save_path`，不接收文件流、不写入文件内容。
- √ 明确文件职责归属：backend 负责文件接收、原始文件存储、文件记录入库和权限控制；agent 只接收 `file_id`、`storage_uri`、`save_path` 等稳定文件引用并读取解析。

### Step 5：解析层与失败回退链路

Step 5 的解析层只负责把 Step 4 的文件感知结果转成 `ParsedDocument`、`DocumentBlock`、源文档定位和解析告警，不负责语义切片、RAG、审阅 prompt 或 LLM 审阅。切片层仍放在 Step 6，不能把解析得到的段落直接等同为审阅分块。

解析后文本适合做短期二次存储，但不应只存单一纯文本。推荐把结构化解析 JSON 作为主产物，保存段落、表格、图片占位、页标、版面序号和源文本偏移；再派生一份面向切片的 Markdown 或纯文本视图。当前基础重构阶段先由 agent 返回结构化解析结果，真实持久化仍由 backend 或后续解析产物存储边界负责，避免在 parser 内部直接写业务存储。

旧版本 DOC/DOCX 解析能力不足以作为新解析层直接复用：`read_docx_text` 和 `docx2text` 会先读全部段落再读全部表格，容易打乱原文顺序；表格语义、合并单元格、图片、样式和页标定位缺失；`docx2md` 保留结构更多，但包含临时图片写入和上传副作用，也没有稳定页码映射。后续只能提取可复用思路，不能把旧函数整体作为新 agent 解析入口。
...
- 【】在下一步评估并引入 PDF 解析包，不在 DOC/DOCX 初始开发中混入 PDF 新依赖。
- 【】验证普通 PDF 使用 MinerU + OCR + 文字处理的解析效果。
- 【】验证 DeepSeek OCR 对复杂表格的识别精度，判断是否需要 AI 补全。
- 【】将 PDF 图片、表格等版面元素清洗为纯文本，并记录映射回源文档的位置。
- 【】为扫描件 PDF 和普通 PDF 分别设计空文本、低置信度、解析异常、外部服务不可用时的失败回退策略。
- 【】形成 PDF 解析评测记录和 fallback 策略说明。
...
- 第四阶段在 agent 模块中做可运行的基础链路，用假模型和样例文本验证结构。
...
- backend 与 agent 分离部署后，如果文件实际存储仍落在 backend，则必须通过共享存储、对象存储或文件服务传递稳定文件引用，不能传本机私有路径。
...
| 2026-05-10 | 实现 Step 3 契约层代码与结构测试 | 已完成 | 在 `src/agent/contracts/` 实现 Pydantic 契约类，并新增 `tests/test_agent_contracts.py` 验证字段承接、默认值、序列化和禁止字段。 |
| 2026-05-10 | 实现 Step 4 文档感知层代码与结构测试 | 已完成 | 在 `src/agent/document_intake/` 实现存储安全、格式分流、PDF 物理分类、版面占位标记和感知编排，并新增 `tests/test_document_intake_sensing.py`。 |
| 2026-05-10 | 标记 Step 4 未完成接线和存储边界 | 待完成 | 当前未接入 FastAPI 上传接口，未实现新 backend 真实文件写入；agent 感知层暂只处理已保存文件路径和路由标记。 |
| 2026-05-10 | 明确文件存储职责归属 | 已完成 | backend 负责文件接收、原始文件存储、文件记录入库和权限控制；agent 只接收稳定文件引用并读取解析。 |
| 2026-05-10 | 收敛 Step 4 为 agent 文件引用感知 | 已完成 | 移除 `safe_filename`、正文输出目标路径和归一化输出路径等 backend 存储产物字段，只保留 `file_id`、`storage_uri`、`save_path` 等稳定引用及感知结果。 |
